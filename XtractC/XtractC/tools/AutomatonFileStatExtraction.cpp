...
AutomatonFileStatExtraction::AutomatonFileStatExtraction()
	: BehavioralMachine()
{
	// Create states
...
bool AutomatonFileStatExtraction::isFileEmpty() const
{
...
uint32_t AutomatonFileStatExtraction::lineCount() const
{
	if (isFileEmpty()) {
		return 0;
	} 
...
uint32_t AutomatonFileStatExtraction::spaceCount() const
{
	if (isFileEmpty()) {
		return 0;
	}
...
uint32_t AutomatonFileStatExtraction::characterCount() const
{
	if (isFileEmpty()) {
		return 0;
	}
